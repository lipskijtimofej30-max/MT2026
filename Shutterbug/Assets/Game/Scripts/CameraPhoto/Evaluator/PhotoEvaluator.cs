...
namespace Game.Scripts
{
    ...
    {
        public PhotoScore CalculateScore(BaseAnimalBrain animalBrain, CinemachineVirtualCamera camera)
        {
            ...
            if (animalBrain.StateMachine == null)
            {
                Debug.LogError("CalculateScore: StateMachine равен null");
                return photoScore;
            }

            photoScore.PoseMultiplier = animalBrain.StateMachine.CurrentState.GetStateMultiplier();
            
            float distance = Vector3.Distance(animalBrain.transform.position, camera.transform.position);
            photoScore.SizePoints = Mathf.RoundToInt(Mathf.Clamp(100000f/(distance * camera.m_Lens.FieldOfView), 0.1f, 500f));
            
            var viewportPosition = Camera.main.WorldToViewportPoint(animalBrain.transform.position);
